...
notesRouter.post('/users/:userId/notes/:noteId', async (req, res) => {
    ...
    let newNote = {UserID: userId, NoteID: noteId, NoteTitle: noteTitle, NoteBody: noteBody, NoteTags: noteTags}
...
notesRouter.put('/users/:userId/notes/:noteId', async (req, res) => {
    ...
    try {
        const db = database.mongoDB;
        await db.collection('Notes').updateOne({UserID: userId, NoteID: noteId}, {$set: edits});
        error = 'Note updated';
    }
    catch (e) {
        error = "Server error:\n" + e.toString();
